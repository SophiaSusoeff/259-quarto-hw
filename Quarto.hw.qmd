...
```
MComment: Formatting looks good! Note, I'm not seeing the ggplots in the final HTML - check and make sure those are still being printed (may need to change r output = True)
...
```{r output = FALSE}
auc <- read_csv("data_raw/auc_bystim.csv")
stim_levels <- 1:7
...
```{r output = FALSE}
ppt <- read_csv("data_raw/participants_info_full_headers.csv") %>% 
...
```{r output = FALSE}
ds %>% write_csv("data_cleaned/cleaned.csv")
```
